...
void setThreadPriority(pthread_t inThread, uint32_t inPriority, bool inIsFixed) {
    ...
    if (inPriority == 96) {
        ...
        auto err = thread_policy_set(pthread_mach_thread_np(threadToAffect), THREAD_TIME_CONSTRAINT_POLICY, (thread_policy_t) &theTCPolicy,
        ...
        thread_policy_set(pthread_mach_thread_np(threadToAffect), THREAD_EXTENDED_POLICY, (thread_policy_t) &theFixedPolicy,
                          THREAD_EXTENDED_POLICY_COUNT);

        // [2] SET PRECEDENCE
        relativePriority = inPriority;
        thePrecedencePolicy.importance = relativePriority;
        thread_policy_set(pthread_mach_thread_np(threadToAffect), THREAD_PRECEDENCE_POLICY, (thread_policy_t) &thePrecedencePolicy,
